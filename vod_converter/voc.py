...
class VOCEgestor(Egestor):
    ...
    def egest(self, image_detections, root):
        ...
        for image_detection in image_detections:
            ...
            shutil.copyfile(image['path'], "%s/%d.%s" % (images_path, int(image_id), src_extension))
            ...
            # add_text_node(xml_root, 'filename', "%d.%s" % (int(image_id), src_extension))
            ...
            ET.SubElement(annotation, "filename").text = str(int(image_id)) + '.' + src_extension
            ...
            for detection in image_detection['detections']:
                ...
                ET.SubElement(object, "name").text = "boat"
                ET.SubElement(object, "pose").text = "Unspecified"
                ET.SubElement(object, "truncated").text = "0"
                ET.SubElement(object, "difficult").text = "0"

                bndbox = ET.SubElement(object, "bndbox")
                ET.SubElement(bndbox, "xmin").text = str(int(detection['left']))
                ET.SubElement(bndbox, "ymin").text = str(int(detection['top']))
                ET.SubElement(bndbox, "xmax").text = str(int(detection['right']))
                ET.SubElement(bndbox, "ymax").text = str(int(detection['bottom']))
            ...
            print('before writting to xml')
            print(annotations_path)
            print(int(image_id))
            print
            ET.ElementTree(annotation).write("%s/%d.xml" % (annotations_path, int(image_id)))
